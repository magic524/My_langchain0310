...
from .review_types import ContractDataset, DatasetPayload
...
def _resolve_output_md(run_root: Path, sample_id: str, output_md: str) -> Path:
    """Resolve a migrated `output.md` path from `run_summary.json`."""

    output_md_path = Path(output_md)
    if output_md_path.exists():
        return output_md_path.resolve()

    fallback_path = (run_root / Path(sample_id) / "output.md").resolve()
...
def build_dataset(
    ...
    *,
    ...
    run_root = resolve_run_root(project_root, run_id, explicit_run_root=word2md_run_root)
    ...
    for item in summary.get("results", []):
        ...
        by_contract.setdefault(contract_id, {})[sample_id] = _resolve_output_md(run_root, sample_id, output_md)
